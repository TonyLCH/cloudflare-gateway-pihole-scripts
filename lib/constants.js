...
const MAX_CLOUDFLARE_LIST_ITEM_LIMIT = 300000;
const configuredListItemLimitRaw = process.env.CLOUDFLARE_LIST_ITEM_LIMIT;
const configuredListItemLimit = configuredListItemLimitRaw
  ? Number.parseInt(configuredListItemLimitRaw, 10)
  : Number.NaN;
if (!Number.isNaN(configuredListItemLimit) && configuredListItemLimit > MAX_CLOUDFLARE_LIST_ITEM_LIMIT) {
  console.warn(`CLOUDFLARE_LIST_ITEM_LIMIT is set to ${configuredListItemLimit}, but Cloudflare Gateway supports at most ${MAX_CLOUDFLARE_LIST_ITEM_LIMIT} domains for this setup. Limiting to ${MAX_CLOUDFLARE_LIST_ITEM_LIMIT}.`);
}
export const LIST_ITEM_LIMIT = Number.isNaN(configuredListItemLimit)
  ? MAX_CLOUDFLARE_LIST_ITEM_LIMIT
  : Math.min(configuredListItemLimit, MAX_CLOUDFLARE_LIST_ITEM_LIMIT);
